...
func convertFrameToASCII(img image.Image, width uint, usecolor bool) string {
	newHeight := uint(float64(img.Bounds().Dy()) * (float64(width) / float64(img.Bounds().Dx())) / 1.5)
...
func main() {
	...
	for {
		...
		goimg, _ := img.ToImage()
		ascii := convertFrameToASCII(goimg, 240, false) // You can replace 80 with your desired width
